"""
This script solves the BA1A problem (Counting frequency of Motif in DNA)
...
for pair in batched(lines_stream, 2):
    ...
    for index in range(len(dna) - motif_len + 1):
        if dna[index : index + motif_len] == motif:
            count_motif += 1
    print(count_motif) # new line for the next potential pair
...
8. So here we counting frequency of motif, not index of locations = that's why we used 'count motif' in loop
"""